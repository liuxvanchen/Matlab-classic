...
base_pre_folder = 'F:\clip_pre';

% 初始化存储数据的cell数组，用于120个月的数据
months = 1:468; % 120个月的数据
years = 1982:2020; % 年份范围
lai_data_cell = cell(1, length(months));
pre_data_cell = cell(1, length(months));
...
for month_idx = 1:length(months)
    ...
    pre_folder = fullfile(base_pre_folder, num2str(year));
    
    % 构建文件名
    lai_file = fullfile(lai_folder, sprintf('mean_lai%04d%02d.tif', year, month));
    pre_file = fullfile(pre_folder, sprintf('pre%04d_%02d_0.05.tif', year, month));
    ...
    pre_data_cell{month_idx} = pre_values;
end

% 将单元数组转换为数组
lai_data = cat(3, lai_data_cell{:});
pre_data = cat(3, pre_data_cell{:});
...
% 输出平均值
fprintf('选定区域降水-LAI的1982-2020年相关系数平均值为: %.4f\n', average_coefficient);
...
fprintf('选定区域降水-LAI的1982-2020年人工林（33）相关系数平均值为: %.4f\n', average_coefficient33);
...
fprintf('选定区域降水-LAI的1982-2020年天然林（2）相关系数平均值为: %.4f\n', average_coefficient2);
