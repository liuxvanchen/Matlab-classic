% 文件夹路径
lai_folder = 'F:\clip_lai\2019\';
pre_folder = 'F:\clip_tmp\2019\';
...
% 遍历每个月份
for month = months
    lai_file = fullfile(lai_folder, sprintf('mean_lai2019%02d.tif', month));
    pre_file = fullfile(pre_folder, sprintf('tmp2019_%02d_0.05.tif', month));
...
% 对每个像元位置进行相关性分析
for i = 111:size(lai_data, 1)
    for j = 305:size(lai_data, 2)
