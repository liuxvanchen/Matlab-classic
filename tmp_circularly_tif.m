...
% 定义路径
ncRootPath = 'E:\人工林数据\气温1000\82-20'; % NC文件的根目录
outputRootPath = 'F:\Tmp'; % 输出文件的根目录

% 循环处理每一年的数据
for year = 1982:2020
    % 构建当前年份的NC文件路径
    ncFileName = sprintf('tmp_%d.nc', year);
    ...
    for month = 1:12
        % 读取当前月份的数据
        data = ncread(ncFilePath, 'tmp', [1, 1, month], [Inf, Inf, 1]);
        ...
        % 构建当前月份的输出文件路径
        outputFileName = sprintf('tmp_%d_%02d.tif', year, month);
        outputFilePath = fullfile(outputYearPath, outputFileName);
        
        % 读取参考Geotiff文件以获取地理参考信息
        [A, R] = geotiffread("F:\pre_tmp_test\标准tif\tmp_Layer1.tif");
        info = geotiffinfo("F:\pre_tmp_test\标准tif\tmp_Layer1.tif");
